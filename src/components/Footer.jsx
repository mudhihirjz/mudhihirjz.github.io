...
export default function Footer() {
  ...
  return (
    <footer className="glass-panel" style={{ padding: '40px', marginTop: '20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '32px', marginBottom: '32px' }}>
        ...
        <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
          {['Home', 'About', 'Services', 'Projects', 'Skills', 'Contact'].map(link => (
            <a key={link} href={`#${link.toLowerCase()}`} style={{ color: 'var(--text-muted)', textDecoration: 'none', fontSize: '14px', transition: 'color 0.3s' }} onMouseEnter={(e) => e.target.style.color = 'white'} onMouseLeave={(e) => e.target.style.color = 'var(--text-muted)'}>{link}</a>
          ))}
        </div>

        <div style={{ display: 'flex', gap: '16px' }}>
          {socials.map((social, idx) => (
            <button key={idx} className="glass-pill" style={{ padding: '10px', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }} aria-label={social.label}>
              {social.icon}
            </button>
          ))}
        </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid var(--border-glass)', paddingTop: '24px', flexWrap: 'wrap', gap: '16px' }}>
        ...
        <p className="handwritten" style={{ margin: 0, fontSize: '20px' }}>Tech for a better tomorrow</p>
      </div>
    </footer>
  );
}